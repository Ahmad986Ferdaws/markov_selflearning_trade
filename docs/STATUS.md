...
| **Kalman pairs framework** | **Done — built to a 14-section external spec, 16/16 mandatory tests** | [20-kalman-pairs.md](20-kalman-pairs.md), `kalman/` package (`core/data/pairs/strategy/ledger/research/cli`), `kalman-cli`. Joseph-form filter (no inversions, PSD-guarded, S>0 checked, missing-obs explicit, separate q_beta/q_alpha), pre-update-innovation signal, diffuse/OLS init, fixed+adaptive Q (lagged vol, clipped), dt-aware local-linear-trend module. Two-leg cash-and-holdings ledger (next-open fills, costs on traded notional, borrow, freeze/rehedge modes) — the `position*spread.diff()` shortcut is banned and test-enforced absent. Training-only likelihood calibration + sensitivity surface; hysteretic state machine with 5 gates; causal health monitoring (NIS/CUSUM/beta-uncertainty — never raw covariance trace); chronological walk-forward: train→validation (predeclared grid)→untouched test vs 5 baselines through the identical engine. **Validated at exact specification: ~95% coverage of the filter's own posterior uncertainty.** Honest findings: SPY/QQQ fails Engle-Granger upfront (p≈0.21–0.63); 15-fold/20y walk-forward verdict: **no robust edge after costs** (best variant Sharpe 0.09). +26 acceptance tests → **90 passing**. |
